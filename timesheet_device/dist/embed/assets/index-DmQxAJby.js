...
const LABELS_ENDPOINT = (() => {
  const override = __vite_import_meta_env__ == null ? void 0 : __vite_import_meta_env__.VITE_DEVICE_LABELS_URL;
...
const resolveLabel = (labels, side) => {
  var _a;
  const entry = (_a = labels[side]) == null ? void 0 : _a.trim();
  return entry && entry.length > 0 ? entry : `${DEFAULT_LABEL_PREFIX} ${side}`;
};
const parseTimestampMs = (payload) => {
...
const formatDurationForInput = (ms) => formatDuration(ms);
const parseDurationInput = (raw) => {
  ...
  if (parts.length > 1) {
    ...
    let seconds = 0;
    if (numeric.length === 3) {
      [hours, minutes2, seconds] = numeric;
    } else {
      [minutes2, seconds] = numeric;
    }
    return Math.max(0, hours * 3600 + minutes2 * 60 + seconds) * 1e3;
  }
...
function App() {
  ...
  const [editingDurationValue, setEditingDurationValue] = reactExports.useState("");
  const [editingLabelValue, setEditingLabelValue] = reactExports.useState("");
  const [editError, setEditError] = reactExports.useState(null);
  const [expandedDates, setExpandedDates] = reactExports.useState(() => [toDateKey(Date.now())]);
  const [rangeMode, setRangeMode] = reactExports.useState("week");
  const [customStart, setCustomStart] = reactExports.useState(toDateKey(Date.now()));
  const [customEnd, setCustomEnd] = reactExports.useState(toDateKey(Date.now()));
  const [downloadError, setDownloadError] = reactExports.useState(null);
  const [labels, setLabels] = reactExports.useState(() => loadStoredLabels());
  const labelsRef = reactExports.useRef(labels);
  const suppressLabelBroadcastRef = reactExports.useRef(false);
  const labelSaveTimeoutsRef = reactExports.useRef({});
  const timelineRef = reactExports.useRef(createTimelineState());
  const [historyReady, setHistoryReady] = reactExports.useState(false);
  const addDurationToLog = reactExports.useCallback((label, startMs, endMs) => {
    if (endMs <= startMs) {
      return;
    }
    setActivityLog((prev) => {
  ...
  const resetActivityState = reactExports.useCallback(() => {
    timelineRef.current = createTimelineState();
    setActivityLog(() => ({}));
  }, []);
  const handleReading = reactExports.useCallback(
    (payload) => {
  ...
  reactExports.useEffect(() => {
    ...
    const loadRemoteLabels = async () => {
      try {
        ...
        console.error("[timesheet-app] failed to load labels from server", error2);
      }
    };
    loadRemoteLabels();
    return () => {
  ...
  const getActiveContribution = (dateKey, label) => {
    const state = timelineRef.current;
  ...
  const submitLabelUpdate = reactExports.useCallback(async (side, value) => {
    try {
      ...
      console.error(`[timesheet-app] failed to persist label for side ${side}`, error2);
    }
  }, []);
  const scheduleLabelSave = reactExports.useCallback((side, value) => {
  ...
  const getRowsForDate = reactExports.useCallback(
    (dateKey) => {
  ...
  const sortedDates = reactExports.useMemo(() => {
  ...
  const beginEdit = reactExports.useCallback((dateKey, label, totalMs) => {
    const state = timelineRef.current;
  ...
  const handleEditDurationChange = reactExports.useCallback((event) => {
    setEditingDurationValue(event.target.value);
  }, []);
  const handleEditLabelChange = reactExports.useCallback((event) => {
  ...
  const handleEditSave = reactExports.useCallback(() => {
    if (!editing) {
      return;
    }
  ...
  const toggleDate = reactExports.useCallback((dateKey) => {
    setExpandedDates((prev) => prev.includes(dateKey) ? prev.filter((key) => key !== dateKey) : [...prev, dateKey]);
  }, []);
  const getRangeBounds = () => {
    if (rangeMode === "week") {
      const { start: todayStart, end: end2 } = getDateBounds(activeDateKey);
      const startDate2 = new Date(todayStart);
      startDate2.setDate(startDate2.getDate() - 6);
      const start2 = new Date(startDate2.getFullYear(), startDate2.getMonth(), startDate2.getDate()).getTime();
      return { start: start2, end: end2 };
    }
    if (rangeMode === "month") {
      const date = parseDateKey(activeDateKey);
      const start2 = new Date(date.getFullYear(), date.getMonth(), 1).getTime();
      const end2 = new Date(date.getFullYear(), date.getMonth() + 1, 1).getTime();
      return { start: start2, end: end2 };
    }
  ...
  const handleDownloadCsv = reactExports.useCallback(() => {
  ...
  const activeSide = (latest == null ? void 0 : latest.side) ?? null;
  ...
  const sourceTimestamp = (latest == null ? void 0 : latest.imu_timestamp_text) ?? (latest == null ? void 0 : latest.imu_timestamp_iso) ?? (latest == null ? void 0 : latest.received_at) ?? null;
  const postHeight = reactExports.useCallback(() => {
    var _a, _b, _c;
    if (typeof window === "undefined") {
      return;
    }
    ...
    const rawHeight = ((_a = doc.documentElement) == null ? void 0 : _a.scrollHeight) || ((_b = doc.body) == null ? void 0 : _b.scrollHeight) || window.innerHeight;
    const height = Math.max(320, Math.min(Math.ceil(rawHeight), 4e3));
    (_c = window.parent) == null ? void 0 : _c.postMessage({ type: "EMBED_HEIGHT", height }, "*");
  }, []);
  reactExports.useEffect(() => {
    postHeight();
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "app-shell", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "app-header", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { children: "Do-Decahedron Orientation" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "status-line", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "status-label", children: "Latest IMU update:" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "status-value", children: formatTimestamp(sourceTimestamp) })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "status-line", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "status-label", children: "Current activity:" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "status-value", children: activeLabel ?? "Waiting for data?" })
      ] }),
      error ? /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "error-text", children: [
        "Error: ",
        error
      ] }) : null
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "activity-summary", "aria-live": "polite", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "Activity Log" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "range-controls", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "range-option", children: [
          "Range",
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", className: "download-button", onClick: handleDownloadCsv, children: "Download CSV" })
      ] }),
      downloadError ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "error-text", children: downloadError }) : null,
      dateGroups.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "placeholder-text", children: "No activity recorded yet." }) : dateGroups.map(({ dateKey, rows, totalMs }) => {
        const isExpanded = expandedDates.includes(dateKey);
        const state = timelineRef.current;
        return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "date-group", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "button",
            {
              type: "button",
              ...
              children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "date-header__title", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "date-label", children: formatDateDisplay(dateKey) }),
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "date-summary", children: [
                    rows.length,
                    " activit",
                    rows.length === 1 ? "y" : "ies",
                    " - ",
                ...
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "date-header__icon", children: isExpanded ? "−" : "+" })
              ]
            }
          ),
          isExpanded ? /* @__PURE__ */ jsxRuntimeExports.jsxs("table", { className: "activity-table", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("thead", { children: /* @__PURE__ */ jsxRuntimeExports.jsxs("tr", { children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("th", { scope: "col", children: "Activity" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("th", { scope: "col", children: "Total Duration" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("th", { scope: "col", className: "actions-heading", children: "Actions" })
            ] }) }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("tbody", { children: rows.map((row) => {
              const isEditing = (editing == null ? void 0 : editing.dateKey) === dateKey && editing.originalLabel === row.label;
              ...
              return /* @__PURE__ */ jsxRuntimeExports.jsxs("tr", { children: [
                ...
                ) : /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: formatDuration(row.totalMs) }) }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "actions-cell", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "action-buttons", children: isEditing ? /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx(
                    "button",
                    {
                      type: "button",
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", className: "icon-button", onClick: handleEditCancel, children: "Cancel" })
      ...
      editError ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "error-text", children: editError }) : null
    ] })
  ] });
}
...
//# sourceMappingURL=index-DmQxAJby.js.map
